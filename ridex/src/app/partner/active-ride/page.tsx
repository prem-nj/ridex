...
export default function Page() {
    ...
    const handleSendDropOtp = async () => {
        ...
        try {
            console.log("📤 Sending drop OTP:", booking._id)
            ...
            // Open OTP input after OTP has been generated
            setDropOtpMode(true)
            setDropOtp("")
    
        } catch (error: any) {
            console.log("❌ SEND DROP OTP ERROR:", error.response?.data)
    
            setDropOtpError(
                error.response?.data?.message ||
                "Failed to send drop OTP"
            )
        } finally {
    ...
    const handleVerifyDropOtp = async () => {
        ...
        if (dropOtp.length !== 4) {
            setDropOtpError("Please enter 4 digit OTP")
            return
        }
        ...
        try {
            console.log(" VERIFYING DROP OTP:", {
                bookingId: booking._id,
                otp: dropOtp,
            })
            ...
            console.log("❌ VERIFY DROP OTP ERROR:", error.response?.data)
                                ...
                                >
                                    ...
                                    <div className='p-4 space-y-3'>
                                        ...
                                        <div className='flex gap-2'>
                                            ...
                                            >
                                                {loadingOtp ? <span>Verifying...</span> : <span>Verify OTP</span>}
                                            </button>
                                        </div>
                                    </div>
                                </motion.div>
                                    ...
                                    <div className='p-4 space-y-3'>
                                        ...
                                        <div className='flex gap-2'>
                                            ...
                                            <button
                                                ...
                                                className="flex-1 bg-zinc-900 hover:bg-zinc-800 disabled:opacity-40 text-white py-2.5 rounded-xl text-sm font-bold active:scale-[0.97] transition-all"
                                            >
                                                {loadingDropOtp ? <span>Verifying...</span> : <span>Verify OTP</span>}
                                            </button>
                                        </div>
                                    </div>
                                </motion.div>
